...
class AssetHeaderGen:
    ...
    def gen_class_header(self):
        ...
        for Vec, Name, Descr in self.VectorParams:
            delim = "" if i == nargs - 1 else ","
            vsize = len(Vec)
            typedarglist += "const Eigen::Matrix<double,{1:},1> & {0:} {2:}".format(
                str(Name), vsize, delim
            )
            untypearglist += "{0:} {2:}".format(str(Name), vsize, delim)

            ctorbody += "\n\t this->{0:}={0:};".format(str(Name))
            members += "\n\t Eigen::Matrix<double,{1:},1> {0:}; // {2:}".format(
                str(Name), vsize, Descr
            )
            i += 1
        ...
        for Mat, Name, Descr in self.MatrixParams:
            ...
            rows = len(Mat)
            cols = len(Mat[0])
            typedarglist += "const Eigen::Matrix<double,{1:},{2:}> & {0:} {3:}".format(
                str(Name), rows, cols, delim
            )
            untypearglist += " {0:}{3:}".format(str(Name), rows, cols, delim)
            ctorbody += "\n\t this->{0:}={0:};".format(str(Name))
            members += "\n\t Eigen::Matrix<double,{1:},{2:}> {0:}; // {3:}".format(
                str(Name), rows, cols, Descr
            )
            i += 1
