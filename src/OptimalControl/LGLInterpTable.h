...
struct LGLInterpTable {
    ...
    std::vector<Eigen::VectorXd> NDequidist(Eigen::VectorXd spacing, int dnum, double low,
                                            double high) { // 0 to 1;
    ...
    std::vector<Eigen::VectorXd> NDdistribute(Eigen::VectorXd ndspacing, Eigen::VectorXd ndtimes,
    ...
    void DeboorMeshError(Eigen::VectorXd &tsnd, Eigen::MatrixXd &mesh_errors,
    ...
    template <class Scalar> Eigen::Matrix<Scalar, -1, 2> InterpolateDeriv(Scalar tglobal) const {
        Eigen::Matrix<Scalar, -1, 2> fx(this->XtUVars, 2);
        fx.setZero();
        this->InterpolateDerivRef(tglobal, fx);
        return fx;
    }
    template <class Scalar> void FindBlock(Scalar tglobal, Scalar &tnd, int &element) const {
        if (this->EvenData) {
        ...
        double tb0 =
            this->XtUData.middleCols((this->BlockSize - 1) * element, this->BlockSize)(axis, 0);
        double tbf = this->XtUData.middleCols((this->BlockSize - 1) * element,
    ...
    void InterpIthBlock(Scalar t, const Eigen::MatrixBase<OutType> &fx, int i) const {
        return this->InterpBlock(
            t, fx, this->XtUData.middleCols((this->BlockSize - 1) * i, this->BlockSize),
    ...
    template <class Scalar> int CheckIthBlock(Scalar tglob, int i) const {
        Scalar t0 = this->XtUData.middleCols((this->BlockSize - 1) * i, this->BlockSize)(axis, 0);
        Scalar tf = this->XtUData.middleCols((this->BlockSize - 1) * i,
                                             this->BlockSize)(axis, this->BlockSize - 1);
