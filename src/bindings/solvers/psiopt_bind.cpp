...
void TychoBind<PSIOPT>::build(nb::module_ &m) {
    ...
    BIND_SETTINGS_RW(
        ...
        "acceptance strategy's per-barrier-subproblem state (the filter/funnel's stored "
        "bounds), exactly as a new barrier subproblem does in Ipopt. The funnel/filter "
