...
struct MatrixFunctionProduct_Impl
                ...
                if constexpr (M1Major == Eigen::ColMajor) {
                    for (int j = 0; j < m1cols_m2rows; j++) {
                        ...
                        this->matrix_func1.right_jacobian_domain_product(
                            jx.template middleRows<M1Rows>(m1rows * i, m1rows), dvec.asDiagonal(),
    ...
    inline void compute_jacobian_adjointgradient_adjointhessian_impl(
        ...
        auto Impl = [&](auto &fxm1, auto &adjv1, auto &jxm1, auto &fxm2, auto &adjv2, auto &jxm2,
                        auto &gxm2, auto &hxm2, auto &fxmd, auto &dvec, auto &jttemp) {
            ...
            if constexpr (M2Major == Eigen::ColMajor) {
                ...
                for (int i = 0; i < m1cols_m2rows; i++) {
                    for (int j = 0; j < m2cols; j++) {
                        adjv2[m2cols * i + j] +=
                            m1.col(i).dot(adjvars.template segment<M1Rows>(m1rows * j, m1rows));
                    }
                }
            }
            ...
            this->matrix_func2.compute_jacobian_adjointgradient_adjointhessian(x, fxm2, jxm2, gxm2,
                                                                               hxm2, adjv2);
            ...
            if constexpr (M2Major == Eigen::ColMajor) {
                ...
                if constexpr (M1Major == Eigen::ColMajor) {
                    ...
                    for (int i = 0; i < m1cols_m2rows; i++) {
                        this->matrix_func1.right_jacobian_domain_product(
                            jttemp.template middleRows<M2Cols>(m2cols * i, m2cols), lmat,
                    ...
                    for (int i = 0; i < m1rows; i++) {
                        ...
                        for (int j = 0; j < m1cols_m2rows; j++) {
                            ...
                            this->matrix_func1.right_jacobian_domain_product(
                                jttemp.template middleRows<M2Cols>(m2cols * j, m2cols), lmat.col(i),
                                jxm1.row(m1cols_m2rows * i + j), PlusEqualsAssignment(),
                                std::bool_constant<false>());
                        }
                    }
                }
