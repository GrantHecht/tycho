...
struct PyVectorFunction : VectorFunction<PyVectorFunction<IRR, ORR>, IRR, ORR, FDiffFwd, FDiffFwd> {
    ...
    using FType =
        std::function<Output<double>(ConstEigenRef<Input<double>>, py::detail::args_proxy)>;
    bool threadSafe = false;
    FType pyfun;
    py::args pyargs;

    PyVectorFunction(int irr, int orr, const FType &f, Input<double> js, Input<double> hs,
                     py::tuple p) {
    ...
    static void Build(py::module &m, const char *name) {
        ...
        if constexpr (ORR != 1) {
            obj.def(py::init<int, int, const FType &, double, double, py::tuple>(),
                    py::arg("IRows"), py::arg("ORows"), py::arg("Func"),
                    py::arg("Jstepsize") = 1.0e-6, py::arg("Hstepsize") = 1.0e-4,
                    py::arg("args") = py::tuple());
        } else {
            obj.def(py::init<int, const FType &, double, double, py::tuple>(), py::arg("IRows"),
                    py::arg("Func"), py::arg("Jstepsize") = 1.0e-6, py::arg("Hstepsize") = 1.0e-4,
                    py::arg("args") = py::tuple());
        }
