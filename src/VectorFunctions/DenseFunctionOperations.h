...
void right_jacobian_product_dynamic_impl(const DomainMatrix &SubDomains,
    ...
    if (sds == 0) {
        ...
        for (int i = 0; i < sds; i++) {
            ...
            ASSET::right_jacobian_product_impl(target_ref.middleCols(start, size), left,
...
void symetric_jacobian_product_dynamic_impl(const DomainMatrix &SubDomains,
    ...
    if (sds == 0 || sds > 1) {
        ...
        for (int i = 0; i < sds; i++) {
            ...
            ASSET::symetric_jacobian_product_impl(target_ref.block(start, start, size, size), left,
...
void accumulate_vector_dynamic_domain_impl(const DomainMatrix &SubDomains,
    ...
    if (sds == 0) {
        ...
        for (int i = 0; i < sds; i++) {
            int start = SubDomains(0, i);
            int size = SubDomains(1, i);
            ASSET::accumulate_impl(target_ref.segment(start, size), right_ref.segment(start, size),
                                   assign);
        }
    }
}
