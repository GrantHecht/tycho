...
def normalize(x):
    return np.copy(x) / np.linalg.norm(x)
...
def Plot(Traj):
    ...
    for i in range(0, 3):
        ...
        for ZP in ZPs:
            T = np.array(ZP).T * 0.3048
            axs[i].plot(
                T[views2[i][0]] + rs, T[views2[i][1]] + rs, zorder=10, color=color
            )

        for ZP in ZNs:
            T = np.array(ZP).T * 0.3048
            axs[i].plot(
                T[views2[i][0]] + rs, T[views2[i][1]] + rs, zorder=2, color=color
            )
        ...
        m = ms[i]
        m.drawcoastlines(ax=axs[i], linewidth=0.25, zorder=5)
        m.fillcontinents(color="coral", lake_color="aqua", ax=axs[i], zorder=5)
        m.drawparallels(np.arange(-90.0, 120.0, 30.0), ax=axs[i], zorder=5)
        m.drawmeridians(np.arange(0.0, 420.0, 60.0), ax=axs[i], zorder=5)
...
if __name__ == "__main__":
    ...
    X0 = ode.make_input(p=pt0, h=ht0, L=np.pi, w=1.0, U=[0.0, 1.0, 0.0], tau=-25.0)
