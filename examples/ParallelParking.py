...
def PlotCar(ode, XtU, ax, col="b"):
    ...
    mat2 = np.array([[np.cos(phi), -np.sin(phi)], [np.sin(phi), np.cos(phi)]])
    ...
    axles[0].set_data(
        np.array([axleloc1[0], axleloc2[0]]) + x,
        np.array([axleloc1[1], axleloc2[1]]) + y,
    )
    ####################################
...
def PlotSlot(ax, k, SL, SW, CL, nxlim=-8, pxlim=8):
    ...
    ax.fill_between(xs, 0, CL, alpha=0.2, color="grey", label="Roadway")

    ax.fill_between(xs, ys, 0, alpha=0.2, color="g", label="Parking Spot")
