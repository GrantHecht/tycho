...
def Plot(TimeOptimal, MassOptimal):
    ...
    ax5.plot(
        np.cos(TO[1]) * TO[0], np.sin(TO[1]) * TO[0], color="r", label="Time Optimal"
    )
    ax5.plot(
        np.cos(MO[1]) * MO[0], np.sin(MO[1]) * MO[0], color="b", label="Mass Optimal"
    )

    ax5.scatter(1, 0, color="k")
    ax5.scatter(
        np.cos(TO[1][-1]) * TO[0][-1],
        np.sin(TO[1][-1]) * TO[0][-1],
        color="r",
        marker="*",
    )
    ax5.scatter(
        np.cos(MO[1][-1]) * MO[0][-1],
        np.sin(MO[1][-1]) * MO[0][-1],
        color="b",
        marker="*",
    )
