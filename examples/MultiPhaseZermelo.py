...
def navigate(Points, vM=1, wF=uniformWind):
    ...
    for i in range(0, numphase):
        ...
        phase.NumPartitions = 8  # Equal to number of physical cores
...
def plot2DTrajList(tList):
    ...
    axes[0].legend()
    plt.tight_layout()
    plt.savefig("Plots/Zermelo/CompareWindModelsLink.svg", dpi=500)
    plt.show()
...
def plot2DTrajListVF(tList, wFunc_num):
    ...
    plt.tight_layout()
    plt.savefig("Plots/Zermelo/CompareBoatSpeedLink.svg", dpi=500)
    plt.show()
...
def compareSpeed():
    ...
    vMRange = np.linspace(1.85, 4.0, num=25)
    trajs = []
    for vM in vMRange:
