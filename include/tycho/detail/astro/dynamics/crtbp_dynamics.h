...
/// @brief Circular Restricted Three-Body Problem (CR3BP) dynamics in the synodic frame (IR=9, OR=6).
///
/// State and acceleration input: [x, y, z, vx, vy, vz, ax, ay, az] in rotating
/// (synodic) coordinates, where [ax, ay, az] is the non-two-body (perturbing)
/// acceleration in the synodic frame — a thrust control, a modeled perturbation,
/// or their sum. mu is the mass ratio of the smaller primary: mu = m2/(m1+m2) ∈ (0,1).
/// Output (state derivative): [vx, vy, vz, x_ddot, y_ddot, z_ddot] in the synodic frame.
...
/// Analytic Jacobian and Hessian generated by utils/codegen_crtbp_dynamics.py.
class CRTBPDynamics
    : public VectorFunction<CRTBPDynamics, 9, 6, DenseDerivativeMode::Analytic, DenseDerivativeMode::Analytic> {
    public:
    using Base = VectorFunction<CRTBPDynamics, 9, 6, DenseDerivativeMode::Analytic, DenseDerivativeMode::Analytic>; ///< @internal CRTP base alias. @endinternal
    ...
    public:
    /// @brief Default constructor; sets mu=0.5.
    CRTBPDynamics() : CRTBPDynamics(0.5) {}

    /// @brief Construct with the given CR3BP mass ratio.
    /// @param[in] mu Mass ratio mu = m2/(m1+m2), must satisfy 0 < mu < 1.
    ...
    /// @brief Set the CR3BP mass ratio.
    ...
    /// @brief Return the current CR3BP mass ratio.
    [[nodiscard]] double mu() const noexcept { return mu_; }

    /// @internal @brief Evaluate the CR3BP dynamics (primal only). @endinternal
    ...
    /// @internal @brief Evaluate the CR3BP dynamics and its Jacobian. @endinternal
    ...
    /// @internal @brief Evaluate CR3BP dynamics, Jacobian, adjoint gradient, and adjoint Hessian. @endinternal
    template <class InType, class OutType, class JacType,
