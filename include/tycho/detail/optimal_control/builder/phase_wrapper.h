...
class Phase {
  public:
    ...
    int add_lu_var_bound(PhaseRegionFlags flag, const std::string &var_name, double lower,
        ...
        return phase_->add_lu_var_bound(flag, resolve_single(var_name, "add_lu_var_bound"), lower, upper,
                                     scale);
    }
    ...
    int add_lower_var_bound(PhaseRegionFlags flag, const std::string &var_name, double lower,
        ...
        return phase_->add_lower_var_bound(flag, resolve_single(var_name, "add_lower_var_bound"), lower,
                                        scale, scale_t);
    }

    int add_upper_var_bound(PhaseRegionFlags flag, const std::string &var_name, double upper,
                         double scale = 1.0, ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_upper_var_bound(flag, resolve_single(var_name, "add_upper_var_bound"), upper,
                                        scale, scale_t);
    }

    int add_value_objective(PhaseRegionFlags flag, const std::string &var_name, double scale,
                          ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_value_objective(flag, resolve_single(var_name, "add_value_objective"), scale,
                                         scale_t);
    }
    ...
    int add_lu_func_bound(PhaseRegionFlags flag, GenericFunction<-1, 1> func,
                       ...
                       double scale = 1.0, ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_lu_func_bound(flag, func, resolve(var_names), lower, upper, scale, scale_t);
    }
    ...
    int add_lower_squared_norm_bound(PhaseRegionFlags flag,
                                 ...
                                 double scale = 1.0,
                                 ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_lower_squared_norm_bound(flag, resolve(var_names), lower, scale, scale_t);
    }
    ...
    int add_upper_squared_norm_bound(PhaseRegionFlags flag,
                                 ...
                                 double scale = 1.0,
                                 ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_upper_squared_norm_bound(flag, resolve(var_names), upper, scale, scale_t);
    }
    ...
    int add_lower_squared_norm_bound(PhaseRegionFlags flag, const Eigen::VectorXi &vars,
                                 double lower, double scale = 1.0,
                                 ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_lower_squared_norm_bound(flag, VarIndexType(vars), lower, scale, scale_t);
    }

    int add_upper_squared_norm_bound(PhaseRegionFlags flag, const Eigen::VectorXi &vars,
                                 double upper, double scale = 1.0,
                                 ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_upper_squared_norm_bound(flag, VarIndexType(vars), upper, scale, scale_t);
    }
    ...
    int add_delta_var_objective(const std::string &var_name, double scale,
                             ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_delta_var_objective(resolve_single(var_name, "add_delta_var_objective"), scale,
                                            scale_t);
    }
    ...
    int add_delta_var_equal_con(const std::string &var_name, double value, double scale = 1.0,
                            ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_delta_var_equal_con(resolve_single(var_name, "add_delta_var_equal_con"), value,
                                           scale, scale_t);
    }
    ...
    int add_lower_delta_var_bound(const std::string &var_name, double lower, double scale = 1.0,
                              ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_lower_delta_var_bound(resolve_single(var_name, "add_lower_delta_var_bound"),
                                             lower, scale, scale_t);
    }
    ...
    int add_upper_delta_var_bound(const std::string &var_name, double upper, double scale = 1.0,
                              ScaleType scale_t = ScaleModes::AUTO) {
        return phase_->add_upper_delta_var_bound(resolve_single(var_name, "add_upper_delta_var_bound"),
                                             upper, scale, scale_t);
    }
    ...
    void set_mesh_error_estimator(MeshErrorEstimators est) { phase_->set_mesh_error_estimator(est); }
    void set_mesh_error_criteria(MeshErrorAggregation crit) { phase_->set_mesh_error_criteria(crit); }
    ...
    std::vector<Eigen::VectorXd> return_costate_traj() const { return phase_->return_costate_traj(); }
    std::vector<Eigen::VectorXd> return_traj_error() const { return phase_->return_traj_error(); }
    bool mesh_converged() const { return phase_->MeshConverged; }
