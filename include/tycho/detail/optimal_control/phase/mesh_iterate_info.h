...
struct MeshIterateInfo {
    ...
    MeshIterateInfo(int numsegs, double tol, const Eigen::VectorXd &times,
        ...
        : numsegs(numsegs), up_numsegs(numsegs), tol(tol), times(times), error(error),
        ...
        for (int i = 0; i < n - 1; i++) {
            this->distintegral[i + 1] =
                this->distintegral[i] +
                (this->distribution[i]) * (this->times[i + 1] - this->times[i]);
        }
    ...
    Eigen::VectorXd calc_bins(int nbins) {
        ...
        for (int i = 1; i < nbins; i++) {
            ...
            auto it =
                std::upper_bound(this->distintegral.cbegin() + elem, this->distintegral.cend(), di);
