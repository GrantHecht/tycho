...
/// @internal Primary template (unspecialised); specialisations hold the actual Butcher tableau
/// data.
template <IVPAlg opt> struct RKCoeffs {};

/// @internal Butcher tableau for RK4Classic (internal dispatch tag only, not runtime-selectable).
template <> struct RKCoeffs<IVPAlg::RK4Classic> {
    ...
    /// @internal Embedded error estimator order (0 = none).
    ...
    /// @internal Total stages referenced by midpoint sum.
    static constexpr int BmidStages = Stages + (LastStageIsFxf ? 0 : 1);

    /// @internal Butcher A matrix (lower-triangular stage coefficients).
    ...
    /// @internal Stage time fractions.
    static constexpr std::array<double, 3> C = {rat(1, 2), rat(1, 2), 1.0};
    /// @internal Main quadrature weights.
    static constexpr std::array<double, 4> B = {rat(1, 6), rat(1, 3), rat(1, 3), rat(1, 6)};
    /// @internal Embedded error weights (unused for this method).
    static constexpr std::array<double, 4> Bhat = {0, 0, 0, 0};
};

/// @internal Butcher tableau for DOPRI54 (Dormand-Prince 5(4), user-selectable).
template <> struct RKCoeffs<IVPAlg::DOPRI54> {
    ...
    /// @internal Extra interpolation stages.
    static constexpr int InterpStages = 0;
    /// @internal Last main stage equals f(xf).
    ...
    /// @internal Embedded error weights.
    ...
    /// @internal Midpoint interpolation weights (2·b_i(0.5)).
...
/// @internal Butcher tableau for DOPRI5 (internal transcription dispatch tag; use DOPRI54 for
/// runtime use).
...
/// @internal Butcher tableau for DOPRI87 (Dormand-Prince 8(7), user-selectable).
...
/// @internal Butcher tableau for Tsit5 (Tsitouras 5(4), user-selectable).
...
/// @internal Butcher tableau for BS3 (Bogacki-Shampine 3(2), user-selectable).
...
/// @internal Butcher tableau for BS3Trans (Bogacki-Shampine 3(2), transcription-only, no dense
/// output).
template <> struct RKCoeffs<IVPAlg::BS3Trans> {
    ...
    /// @internal Embedded error weights (unused).
...
// compute_bmid_bs5.jl).
/// @internal Butcher tableau for BS5 (Bogacki-Shampine 5(4), user-selectable).
template <> struct RKCoeffs<IVPAlg::BS5> {
    static constexpr int Stages = 8;           ///< @internal Number of stages.
    static constexpr int Order = 5;            ///< @internal Method order.
    static constexpr int ErrorOrder = 4;       ///< @internal Embedded error estimator order.
    /// @internal First-same-as-last property (B_8=0; not strict-FSAL).
    ...
    /// @internal Extra interpolation stage coefficients.
    ...
    /// @internal Extra interpolation stage time fractions.
...
/// @internal Butcher tableau for BS5Trans (Bogacki-Shampine 5(4), transcription-only, no dense
/// output).
...
/// @internal Butcher tableau for Vern7 (Verner 7(6), user-selectable).
...
/// @internal Butcher tableau for Vern7Trans (Verner 7(6), transcription-only, no dense output).
...
/// @internal Butcher tableau for Vern8 (Verner 8(7), user-selectable).
...
/// @internal Butcher tableau for Vern8Trans (Verner 8(7), transcription-only, no dense output).
...
/// @internal Butcher tableau for Vern9 (Verner 9(8), user-selectable).
...
/// @internal Butcher tableau for Vern9Trans (Verner 9(8), transcription-only, no dense output).
...
/// @internal Butcher tableau for Tsit5Trans (Tsitouras 5(4), transcription-only, no dense output).
