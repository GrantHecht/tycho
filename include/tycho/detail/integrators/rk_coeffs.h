...
template <> struct RKCoeffs<IVPAlg::DOPRI54> {
    ...
    static constexpr STDarray<STDarray<double, 6>, 6> ACoeffs = {
        ...
        STDarray<double, 6>{19372.0 / 6561.0, -25360.0 / 2187.0, 64448.0 / 6561.0, -212.0 / 729.0,
                            0, 0},
        STDarray<double, 6>{9017.0 / 3168.0, -355.0 / 33.0, 46732 / 5247.0, 49.0 / 176.0,
                            -5103.0 / 18656.0, 0},
        STDarray<double, 6>{35.0 / 384.0, 0.0, 500.0 / 1113.0, 125 / 192.0, -2187.0 / 6784.0,
                            11.0 / 84.0}};

    static constexpr STDarray<double, 6> Times = {1.0 / 5.0, 3.0 / 10.0, 4.0 / 5.0,
                                                  8.0 / 9.0, 1.0,        1.0};
    ...
    static constexpr STDarray<double, 7> MidCoeffs = {
...
template <> struct RKCoeffs<IVPAlg::DOPRI5> {
    ...
    static constexpr STDarray<STDarray<double, 5>, 5> ACoeffs = {
        ...
        STDarray<double, 5>{19372.0 / 6561.0, -25360.0 / 2187.0, 64448.0 / 6561.0, -212.0 / 729.0,
                            0},
        STDarray<double, 5>{9017.0 / 3168.0, -355.0 / 33.0, 46732 / 5247.0, 49.0 / 176.0,
                            -5103.0 / 18656.0}};
...
template <> struct RKCoeffs<IVPAlg::DOPRI87> {
    ...
    static constexpr STDarray<STDarray<double, 12>, 12> ACoeffs = {
        ...
        STDarray<double, 12>{29443841.0 / 614563906.0, 0, 0, 77736538.0 / 692538347.0,
                             -28693883.0 / 1125000000.0, 23124283.0 / 1800000000.0, 0, 0, 0, 0, 0,
                             0},
        STDarray<double, 12>{16016141.0 / 946692911.0, 0, 0, 61564180.0 / 158732637.0,
                             22789713.0 / 633445777.0, 545815736.0 / 2771057229.0,
                             -180193667.0 / 1043307555.0, 0, 0, 0, 0, 0},
        STDarray<double, 12>{39632708.0 / 573591083.0, 0, 0, -433636366.0 / 683701615.0,
                             -421739975.0 / 2616292301.0, 100302831.0 / 723423059.0,
                             790204164.0 / 839813087.0, 800635310.0 / 3783071287.0, 0, 0, 0, 0},

        STDarray<double, 12>{246121993.0 / 1340847787.0, 0, 0, -37695042795.0 / 15268766246.0,
        ...
        STDarray<double, 12>{-1028468189.0 / 846180014.0, 0, 0, 8478235783.0 / 508512852.0,
        ...
        STDarray<double, 12>{185892177.0 / 718116043.0, 0, 0, -3185094517.0 / 667107341.0,
        ...
        STDarray<double, 12>{403863854.0 / 491063109.0, 0, 0, -5068492393.0 / 434740067.0,
    ...
    static constexpr STDarray<double, 12> Times = {1.0 / 18.0,   1.0 / 12.0,
                                                   1.0 / 8,      5.0 / 16.0,
                                                   3.0 / 8.0,    59.0 / 400.0,
    ...
    static constexpr STDarray<double, 14> MidCoeffs = {
