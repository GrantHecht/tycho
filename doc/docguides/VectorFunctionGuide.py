...
def RTNTransform():
    ...
    RTNcoeffs = RTNBasisFunc(
        vf.stack(R, V)
    )  # Does Exactly the same thing as above calls stack on R,V explicitly
...
expensive = (
    1.0
    / (R.normalized().cross(V.normalized_power3()).dot(R + V.cross(R).normalized()))
    ** 3.14
)
...
print(Tab(-0.00001))  # prints a warning
...
PyVfunc = vf.PyVectorFunction(
    InputSize, OutputSize, VFunc, Jstepsize=1.0e-6, Hstepsize=1.0e-4, args=(3, 7)
)  ## a and b will be 2
...
PySfunc = vf.PyScalarFunction(
    InputSize, SFunc, Jstepsize=1.0e-6, Hstepsize=1.0e-5, args=(1, 2, 3)
)
