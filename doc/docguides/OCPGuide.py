...
ocp.addLinkEqualCon(
    ALinkEqualCon(), 0, "Last", XtUvars0, [], SPvars0, 1, "First", XtUvars1, OPvars1, []
)
...
ocp.addLinkEqualCon(SomeFunc, phase2, "Last", range(0, 3), phase3, "First", range(0, 3))


SomeOtherFunc = Args(2).sum() - 1
# Only needs ODEparams from phases 0 and 1
ocp.addLinkEqualCon(SomeOtherFunc, 0, "ODEParams", [0], 1, "ODEParams", [0])
...
for i in range(0, 5):
    ocp.addDirectLinkEqualCon(i, "Last", range(0, 3), i + 1, "First", range(0, 3))
