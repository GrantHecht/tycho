...
ode = TwoBodyLTODE(1, 0.015)
integULaw = ode.integrator("DOPRI87", 0.1, ULaw(0.8), [3, 4, 5])
...
TrajULaw, Events = integULaw.integrate_dense(
    X0t0U0,
    tf,
    [
        (RStop(2), 0, 1),
...
phase.addBoundaryValue("Front", range(0, 7), X0t0U0[0:7])
phase.addLUNormBound("Path", range(7, 10), 0.0001, 1.0)
phase.addLowerNormBound("Path", range(0, 3), 0.99999)
...
phase.addUpperDeltaTimeBound(TrajULaw[-1][6] * 1.00, 0.1)
