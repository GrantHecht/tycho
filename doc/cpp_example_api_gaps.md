...
| DionysusLowThrust | `ODE::integrator()` | Medium | Builder `ODE` has no `integrator()` method. Cannot generate initial guesses via integration. Must use manual lerp or external propagation. | Add `ODE::integrator()` returning a builder-compatible integrator |
...
| OrbitContinuation | `ODE::integrator()` | Medium | Same gap as DionysusLowThrust. Cannot generate orbital IG via integration. | (same fix) |
...
| MinTimeClimbTables | `InterpTable1D`/`InterpTable2D` in ODE | High | Same gap as MinTimeToClimb. Tables exist in C++ (`tycho::oc::InterpTable1D/2D`) but cannot be composed with VF expressions. | Expose InterpTable as VF-composable expression nodes |
