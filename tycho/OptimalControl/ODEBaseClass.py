    ...
    def __init__(self, odefunc, Xvars, Uvars=None, Pvars=None, Vgroups=None):
        ...
        if constsize == False:
            if hasUvars and hasPvars:
                self.ode = _tycho.OptimalControl.ode_x_u_p.ode(
                    odefunc, Xvars, Uvars, Pvars
                )
            elif hasUvars:
                self.ode = _tycho.OptimalControl.ode_x_u.ode(odefunc, Xvars, Uvars)
            elif hasPvars:
                self.ode = _tycho.OptimalControl.ode_x_u_p.ode(odefunc, Xvars, 0, Pvars)
            else:
                self.ode = _tycho.OptimalControl.ode_x.ode(odefunc, Xvars)
