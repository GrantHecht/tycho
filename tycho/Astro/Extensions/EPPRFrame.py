...
from tycho.Astro.DataReadWrite import ReadCopernicusFile, ReadData, WriteData
from tycho.Astro.Extensions.CR3BPFrame import CR3BPFrame
from tycho.Astro.SpiceRead import GetEphemTraj2, PoleVector, SpiceFrameTransform
...
class EPPRFrame(CR3BPFrame):
    def __init__(
        self, P1name, P1mu, P2name, P2mu, Lstar, JD0, JDF, N=3000, SpiceFrame="J2000"
    ):
        ...
        self.P1Data = GetEphemTraj2(
            P1name, JD0, JDF, N, self.lstar, self.tstar, Frame=self.SpiceFrame
        )
        self.P2Data = GetEphemTraj2(
            P2name, JD0, JDF, N, self.lstar, self.tstar, Frame=self.SpiceFrame
        )
    ...
    def NDInertial_to_EPPR(
        self, ITraj, axis=6
    ):  ## assumes already ND and reffed to JD0
    ...
    def GetSpiceBodyEPPRTraj(self, Name, N):
        ITraj = GetEphemTraj2(
            Name, self.JD0, self.JDF, N, self.lstar, self.tstar, Frame=self.SpiceFrame
        )
    ...
    def Add_P2_J2Effect(self, J2c=None, RadP2=None):
        if J2c == None:
            J2c = BProps[self.P2name]["J2"]
        if RadP2 == None:
            RadP2 = BProps[self.P2name]["Radius"]

        self.P2_Rad = RadP2 / self.lstar
        self.P2_J2 = J2c
        self.P2_PoleVectors = self.GetPoleVectors(
            "IAU_" + self.P2name, len(self.P2Data)
        )
        self.P2_PoleFunc = oc.InterpFunction_3(
            oc.LGLInterpTable(3, self.P2_PoleVectors, len(self.P2Data))
        ).vf()

    def Add_P1_J2Effect(self, J2c=None, RadP1=None):
        if J2c == None:
            J2c = BProps[self.P1name]["J2"]
        if RadP1 == None:
            RadP1 = BProps[self.P1name]["Radius"]
        self.P1_Rad = RadP1 / self.lstar
        self.P1_J2 = J2c
        self.P1_PoleVectors = self.GetPoleVectors(
            "IAU_" + self.P1name, len(self.P1Data)
        )
        self.P1_PoleFunc = oc.InterpFunction_3(
            oc.LGLInterpTable(3, self.P1_PoleVectors, len(self.P1Data))
        ).vf()
    ...
    def EPPREOMs(
        ...
        if ActiveAltBodies == "All":
            Names = self.AltBodyNames
        else:
            Names = self.ActiveAltBodies
        for Name in Names:
    ...
    def Copernicus_to_Frame(
        self, Filename, center="P2", SpiceFrame="J2000", Folder="Data"
    ):
        ...
        for T in data:
            ...
            if SpiceFrame != self.SpiceFrame:
                TC[0:6] = SpiceFrameTransform(
                    SpiceFrame, self.SpiceFrame, TC[0:6], TC[7]
                )
    ...
    def CalcFrameData(self):
        ...
        for i in range(0, len(self.BCData)):
            self.BCData[i][0:6] = (
                self.P1mu * self.P1Data[i][0:6] + self.P2mu * self.P2Data[i][0:6]
            ) / (self.P1mu + self.P2mu)
