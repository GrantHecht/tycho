...
class test_InterpTables(unittest.TestCase):
    ...
    def test_Interp2D(self):
        ...
        ys = np.linspace(-1.33 * np.pi, np.pi * 0.9, ny)
    ...
    def test_Interp3D(self):
        ...
        with self.subTest("Even, Even, Even"):
            self.Interp3D_test(
                Func, dFunc, d2Func, xs, ys, zs, xscheck, yscheck, zscheck
            )

        with self.subTest("Even, Uneven, Even"):
            self.Interp3D_test(
                Func, dFunc, d2Func, xs, ysu, zs, xscheck, yscheck, zscheck
            )

        with self.subTest("Uneven, Even, Uneven"):
            self.Interp3D_test(
                Func, dFunc, d2Func, xsu, ys, zsu, xscheck, yscheck, zscheck
            )

        with self.subTest("Even, Even, Even: Cached"):
            self.Interp3D_test(
                Func, dFunc, d2Func, xs, ys, zs, xscheck, yscheck, zscheck, cache=True
            )

        with self.subTest("Even, Uneven, Even: Cached"):
            self.Interp3D_test(
                Func, dFunc, d2Func, xs, ysu, zs, xscheck, yscheck, zscheck, cache=True
            )
    ...
    def test_Interp4D(self):
        ...
        def dFunc(x, y, z, w):
            return -np.array(
                [
                    np.sin(x) * np.cos(y) * np.cos(z) * np.cos(w),
        ...
        def d2Func(x, y, z, w):
            return np.array(
                [
                    [
                        -np.cos(x) * np.cos(y) * np.cos(z) * np.cos(w),
                        np.sin(x) * np.sin(y) * np.cos(z) * np.cos(w),
                        np.sin(x) * np.cos(y) * np.sin(z) * np.cos(w),
                        np.sin(x) * np.cos(y) * np.cos(z) * np.sin(w),
                    ],
                    [
                        np.sin(x) * np.sin(y) * np.cos(z) * np.cos(w),
                        -np.cos(x) * np.cos(y) * np.cos(z) * np.cos(w),
                        np.cos(x) * np.sin(y) * np.sin(z) * np.cos(w),
                        np.cos(x) * np.sin(y) * np.cos(z) * np.sin(w),
                    ],
                    [
                        np.sin(x) * np.cos(y) * np.sin(z) * np.cos(w),
                        np.cos(x) * np.sin(y) * np.sin(z) * np.cos(w),
                        -np.cos(x) * np.cos(y) * np.cos(z) * np.cos(w),
                        np.cos(x) * np.cos(y) * np.sin(z) * np.sin(w),
                    ],
                    [
                        np.sin(x) * np.cos(y) * np.cos(z) * np.sin(w),
                        np.cos(x) * np.sin(y) * np.cos(z) * np.sin(w),
        ...
        ys = np.linspace(-1.33 * np.pi, np.pi * 0.9, ny) / 3
        zs = np.linspace(-1.1 * np.pi, np.pi * 1.06, nz) / 3
        ws = np.linspace(-0.94 * np.pi, np.pi * 1.12, nw) / 3
        ...
        with self.subTest("Even, Even, Even, Even"):
            self.Interp4D_test(
                Func, dFunc, d2Func, xs, ys, zs, ws, xscheck, yscheck, zscheck, wscheck
            )
