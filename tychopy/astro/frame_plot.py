...
class PlotBase:
    ...
    def PlotSTraj(self, mesh, col="b", name="", norm=1, plot=True, Sl=0.06):
        m1 = np.copy(mesh).T
    ...
    def PlotSTraj2(
        self, mesh, col="b", name="", norm=1, plot=True, Sl=0.06, view=[0, 1]
    ):
        m1 = np.copy(mesh).T
        for s in m1[0:-1:norm]:
    ...
    def PlotLTraj(self, mesh, col="b", name="", norm=1, plot=True, Sl=0.06):
        m1 = np.copy(mesh).T
        for s in m1[0:-1:norm]:
    ...
    def PlotS2d(self, mesh, col="b", name="", norm=1, Sl=0.06):
        m1 = np.copy(mesh).T
        for s in m1[0:-1:norm]:
            fdir = s[0:3] + Sl * s[7:10] / np.linalg.norm(s[7:10])
    ...
    def Plot2dAx(
        ...
        for obj in self.Points.values():
            ...
            ax.scatter(
                ...
                marker=obj["Marker"],
                s=obj["Markersize"],
                edgecolors=obj["EdgeColor"],
                linewidths=obj["Markeredgewidth"],
                zorder=3,
            )
        if legend:
            ax.legend()
...
class CRPlot(PlotBase):
    def __init__(self, ode, p1name="Sun", p2name="Earth", p1col="gold", p2col="green"):
        ...
        self.ode = ode
        [[0.985, 0.995], [-0.005, 0.005], [-0.005, 0.005]]
        p = 40
        L1 = {
            "Pos": ode.L1,
            "Color": "magenta",
        ...
        p2pos = ode.P2[0]
        self.BBox["L1P2L2"] = [
            [p2pos - sphoi, p2pos + sphoi],
            [-sphoi, sphoi],
            [-sphoi, sphoi],
        ]
        self.BBox["L1P2L2_wide"] = [
            [p2pos - sphoi * 2, p2pos + sphoi * 2],
            [-sphoi * 2, sphoi * 2],
            [-sphoi * 2, sphoi * 2],
        ]
        ...
        self.BBox["L1"] = [
            [l1pos - sphoi / 2, l1pos + sphoi / 2],
            [-sphoi / 2, sphoi / 2],
            [-sphoi / 3, sphoi / 3],
        ]
        self.BBox["L2"] = [
            [l2pos - sphoi / 2, l2pos + sphoi / 2],
            [-sphoi / 2, sphoi / 2],
            [-sphoi / 2, sphoi / 2],
        ]
        ...
        self.BBox["2deg"] = [
            [pp[0] - sphoi, pp[0] + sphoi],
            [pp[1] - sphoi, pp[1] + sphoi],
            [-sphoi, sphoi],
        ]

        self.BBox["P1P2L5"] = [[0, 1], [-1, 0], [-0.5, 0.5]]
        self.BBox["L5"] = [[0.38, 0.62], [-0.986, -0.746], [-0.1, 0.1]]
        self.BBox["All"] = [[-1, 1], [-1, 1], [-1, 1]]
        self.BBox["offs"] = [
            [0.9848 - sphoi, 0.9848 + sphoi],
            [-sphoi - 0.17, sphoi - 0.17],
            [-sphoi, sphoi],
        ]
        self.NormDensity = 12
        self.NormScales["L1P2L2"] = 0.005
        self.NormScales["2deg"] = 0.03

        self.NormScales["offs"] = 0.005
        self.NormScales["L1P2"] = 0.05
        self.NormScales["P2L2"] = 0.02
        self.NormScales["L1"] = 0.0015
        self.NormScales["L2"] = 0.002
        self.NormScales["P1P2L5"] = 0.07
        self.NormScales["L5"] = 0.002
        self.NormScales["All"] = 0.02
