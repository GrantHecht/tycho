...
class test_MultiPhaseCannon(unittest.TestCase):
    ...
    def problem_impl(self, tmode, nsegs):
        ...
        AscentIG = integ.integrate_dense(
            IG, 60 / Tstar, 1000, lambda x: x[0] * np.sin(x[1]) < 0
        )
        DescentIG = integ.integrate_dense(
            AscentIG[-1], AscentIG[-1][4] + 30 / Tstar, 1000, lambda x: x[2] < 0
        )

        ##########################################################################

        aphase = ode.phase(tmode, AscentIG, nsegs)
        aphase.add_lower_var_bound("ODEParams", 0, 0.01 / Lstar)
