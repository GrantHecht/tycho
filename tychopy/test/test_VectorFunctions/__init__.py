...
class test_VectorFunctions(unittest.TestCase):
    def function_test_impl(
        ...
        self.assertLess(
            max_selfadjointgrad_error, 1.0e-12, "Adjoint gradients do not match"
        )
    ...
    def MatrixOps_impl(self, Ltype, lrows, lcols, Rtype, rrows, rcols):
        ...
        MProdval_truth = np.dot(
            M1val * M1scale + M1shift, M2val * M2scale + M2shift
        ).flatten("F")
        ...
        with self.subTest("Derivatives"):
            self.function_test_impl(
                Fun, Xin, L, Verbose=False, maxjerror=1.0e-5, maxherror=1.0e-5
            )
        ...
        for m1rows in range(1, 10):
            for m1cols_m2rows in range(1, 10):
                for m2cols in range(1, 10):
                    with self.subTest(
                        f" Col * Row {m1rows}, ({m1cols_m2rows},{m2cols})"
                    ):
                        self.MatrixOps_impl(
                            vf.ColMatrix,
                            m1rows,
                    ...
                    with self.subTest(
                        f" Col * Col {m1rows}, ({m1cols_m2rows},{m2cols})"
                    ):
                        self.MatrixOps_impl(
                            vf.ColMatrix,
                            m1rows,
                    ...
                    with self.subTest(
                        f" Row * Col {m1rows}, ({m1cols_m2rows},{m2cols})"
                    ):
                        self.MatrixOps_impl(
                            vf.RowMatrix,
                            m1rows,
                    ...
                    with self.subTest(
                        f" Row * Row {m1rows}, ({m1cols_m2rows},{m2cols})"
                    ):
                        self.MatrixOps_impl(
                            vf.RowMatrix,
                            m1rows,
