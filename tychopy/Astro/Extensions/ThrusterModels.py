...
class SolarSail:
    ...
    def __init__(
        ...
        self.n2 = self.Bf * (1 - self.sbar) * self.rbar + (1 - self.rbar) * (
            self.ef * self.Bf - self.eb * self.Bb
        ) / (self.ef + self.eb)
        self.t1 = 1 - self.sbar * self.rbar

        if Ideal == True:
            self.Normalbeta = self.beta
        else:
            self.Normalbeta = self.beta * (self.n1 + self.n2) / 2.0
    ...
    def ThrustExpr(self, r, n, mu):
        ...
        """
        if self.Ideal == True:
            return self.IdealSailExpr(r, n, mu)
        else:
            return self.MccinnesSailExpr(r, n, mu)
    ...
    def MccinnesSailExpr(self, r, n, mu):
        ...
        sc = self.beta * mu / 2.0
        acc1 = N3DR4 * (
            ((self.n1 * sc) * ndr + (self.n2 * sc) * rn) * n + (self.t1 * sc) * ncrn
        )
