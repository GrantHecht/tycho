...
class CR3BPFrame:
    ...
    def CalcPoints(self):
        ...
        self.L4 = np.array([0.5 - mu, np.sqrt(3.0) / 2.0, 0.0])
        self.L5 = np.array([0.5 - mu, -np.sqrt(3.0) / 2.0, 0.0])
        ...
        while abs(guess - gamma0) > 10e-15:
            gamma0 = guess
            guess = pow(
                ((1.0 - mu) * (gamma0 + 1) * (gamma0 + 1.0))
                / (1.0 + 2.0 * mu + gamma0 * (2.0 + mu + gamma0)),
                1.0 / 3.0,
            )
