...
unobservable through this API, and P2 cannot regress any path. Check 2c is marked **PASS** with
the SDK citation; no probe is required, though the probe sources are retained.
...
| `SparseRefactor` failure | **Keep** the numeric factorization object | Apple documents the failed-numeric object as refactorable (`Solve.h:1568-1576`, state 3), which is exactly PSIOPT's perturb-and-retry loop; the new status guard makes a bad solve impossible, so resetting buys nothing and forfeits the documented cheap recovery |
...
`src/solvers/tycho_solvers.h:19` — i.e. most of the 155 Accelerate-enabled TUs plus the PCH. Those
...
- `static_assert(std::is_same_v<StorageIndex, int>)`. The file already `const_cast`s
  `innerIndexPtr()` to `int *` (`:454`, `:472`) and hands `permutation_.data()` to an `int *`
  field, so this is an existing hard requirement with nothing enforcing it. Pardiso asserts its
  index width in its constructor (`pardiso_interface.h:120`).
- `doAnalysis()` sizes `permutation_` by `std::max(n_rows_, n_cols_)` (`:486`). `fopts.order` is
  the *column* ordering, and `AccelerateQR`/`AccelerateCholeskyAtA` are declared
  `EnforceSquare_ = false`, so a wide matrix would have Accelerate write past the buffer. No-op
  for the square in-tree case.
...
- `warmup_sparse_solver()` sets `fopts.reportError`, and calls `SparseCleanup` on both the symbolic
  ...
  **`_SparseTrap()`**, an abort, because `_SparseDefaultSymbolicFactorOptions` has no
  `.reportError` (`SolveImplementation.h:10-17, 81-94`). Low severity (the warmup matrix is a
  hardcoded 2×2 SPD) but it is a startup-path abort risk and a leak.
...
| `mem_` unit divergence — Accelerate reports factor size in **bytes**, Pardiso reports **nonzeros** in the factor (`iparm_[17]`), both surfaced as `result_.factor_mem_` (`psiopt.cpp:2770-2771`) | Documented; only the narrowing is fixed here |
...
4. `set_order` downgrade asserts the solver still works; it cannot assert *which* ordering ran.
...
- **I4** — reproduce the failed-refactor-then-solve sequence through the *public* interface (the
  existing probe drives the raw C API), confirming the new guard fires and `info()` reports.
...
| Runtime `__builtin_available` gates cannot be tested on this machine (macOS 26.5 host) | The downgrade path is exercised by the `set_order` test; the negative case is inspection-only and documented as such |
