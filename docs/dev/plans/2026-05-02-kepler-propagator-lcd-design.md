...
```
 ...
 SymPy at dev time   │ utils/codegen_kepler_propagator.py│
                     ...
                     │   • struct KeplerPrimal (S → rf,vf)     │
                     │   • struct KeplerResidual (F)           │
...
def KeplerPrimal():
    ...
    return TychoHeaderGen("KeplerPrimal", Func, Xs,
...
The two generated structs (`KeplerPrimal` and `KeplerResidual`) are emitted into the same file `kepler_propagator_closed_form.h` and are **not** registered as Python-visible VFs — they are internal helpers. We invoke their `compute_*_impl` methods directly from the IFT layer.

### 3.4 `kepler_propagator_closed_form.h` — generated (do not edit manually)

Output of §3.3. Header marker: `// Generated by utils/codegen_kepler_propagator.py — do not edit manually.` (TychoHeaderGen already emits this preamble.) Two struct definitions: `KeplerPrimal` and `KeplerResidual`. Both have `compute_impl`, `compute_jacobian_impl`, `compute_jacobian_adjointgradient_adjointhessian_impl` from the existing codegen.
...
3. Call `KeplerPrimal{mu}.compute_jacobian_impl(input, xf, S_jac)` — `S_jac` is the 6×11 *structural* Jacobian (treats X and U_n as independent inputs).
4. Pack `(R0, V0, dt, U1, U2, U3)` into a 10-vector input. (F has no explicit X dependence — see §3.3.)
5. Call `KeplerResidual{mu}.compute_jacobian_impl(input, F_val, F_jac)` — the 1×10 row of structural F-partials.
...
(R0, V0, dt, mu) ──► kepler_lcd_iterate ──► (X*, U0..U3, r0, sigma0, r, sigma)
                                          ──► KeplerPrimal::compute_jacobian_impl    ──► dS/d(R0,V0,dt,X,U_n)
                                          ──► KeplerResidual::compute_jacobian_impl  ──► dF/d(R0,V0,dt,U_n)
                                          ──► IFT layer ──► (rf, vf, J ∈ R^{6×7})
```

**VF adjoint-Hessian call:** same kernel call (no extra U-values needed); the second-derivative codegen entry points (`compute_jacobian_adjointgradient_adjointhessian_impl`) on both `KeplerPrimal` and `KeplerResidual` are invoked. IFT layer applies the second-order chain rule (α-recursion still goes index-down).
