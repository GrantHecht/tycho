...
for rad, col in ((1.0, "0.8"), (2.0, "0.8")):
    ax0.plot(rad * np.cos(ang), rad * np.sin(ang), color=col, lw=1, zorder=1)
ax0.plot(G[0], G[1], color="0.75", ls="--", lw=1, label="initial guess", zorder=2)
ax0.plot(T[0], T[1], color="C0", lw=2, label="time-optimal", zorder=3)
ax0.scatter([X0[0]], [X0[1]], color="k", zorder=5, label="start")
ax0.scatter([Xf[0]], [Xf[1]], color="C3", zorder=5, label="target")
...
ax1.plot(T[6], umag, color="C1", lw=2)
ax1.set(
