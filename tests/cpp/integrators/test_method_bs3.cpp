///////////////////////////////////////////////////////////////////////////////
// BS3 method validation against OrdinaryDiffEq.jl reference
///////////////////////////////////////////////////////////////////////////////
...
class BS3Test : public VectorFunctionFixture {};

TEST_F(BS3Test, TwoBodyMatchesJulia) {
    Kepler kep(MU_EARTH);
    Integrator<Kepler> integ(kep, IVPAlg::BS3, 10.0);
    ...
    auto ref = read_julia_ref(julia_ref_path("bs3_twobody.bin"));

    // 3rd-order method at atol=1e-8, rtol=1e-9: expect ~1e-6 agreement.
    for (int i = 0; i < 6; ++i) {
        EXPECT_NEAR(xf[i], ref.uf[i], 1e-4)
            << "BS3 two-body state[" << i << "] mismatch: Tycho=" << xf[i]
            << " Julia=" << ref.uf[i];
    }
}
...
TEST_F(BS3Test, CR3BPMatchesJulia) {
    ...
    auto ref = read_julia_ref(julia_ref_path("bs3_crtbp.bin"));

    for (int i = 0; i < 6; ++i) {
        EXPECT_NEAR(xf[i], ref.uf[i], 1e-4)
            << "BS3 CR3BP state[" << i << "] mismatch: Tycho=" << xf[i]
            << " Julia=" << ref.uf[i];
    }
}
