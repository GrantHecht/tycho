...
//
// Adaptive-path cases (8-10) use the relative matchers (bound = tol *
// max(|golden|, 1.0), tol = 1e-9): goldens are recorded on x86 and the
// compared values span near-zero integration noise up to km-scale (~7000)
// and O(1e3) Jacobian entries, where the former absolute 1e-12..1e-14
// bounds demanded sub-ULP cross-ISA agreement (observed arm64 drift up to
// 6.0e-11; first caught on Apple Silicon, 2026-07-25). Single-step cases
// (11-12) keep tight absolute bounds — one fixed step stays within a few
// ULPs of its golden (see kTranscriptionAbsTol).
///////////////////////////////////////////////////////////////////////////////
...
TEST_F(RegressionTranscriptionTest, Case08_Jacobian) {
    ...
    expect_vector_match_rel(xf_dyn, golden_xf, 1e-9, "Case08_xf");
    expect_matrix_match_rel(jx, golden_jx, 1e-9, "Case08_Jacobian");
}
...
TEST_F(RegressionTranscriptionTest, Case09_JacobianHessian) {
    ...
    expect_vector_match_rel(xf_dyn, golden_xf, 1e-9, "Case09_xf");
    expect_matrix_match_rel(jx, golden_jx, 1e-9, "Case09_Jacobian");
    expect_matrix_match_rel(hx, golden_hx, 1e-9, "Case09_Hessian");
}
...
TEST_F(RegressionTranscriptionTest, Case10_BatchJacobians) {
    ...
    for (size_t i = 0; i < stm_results.size(); ++i) {
        ...
        expect_matrix_match_rel(jx, golden_jxs[i], 1e-9, "Case10_Jac_" + std::to_string(i));
    }
}
