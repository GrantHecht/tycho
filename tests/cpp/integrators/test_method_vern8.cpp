///////////////////////////////////////////////////////////////////////////////
// Vern8 method validation against OrdinaryDiffEq.jl reference
///////////////////////////////////////////////////////////////////////////////
...
class Vern8Test : public VectorFunctionFixture {};

TEST_F(Vern8Test, TwoBodyMatchesJulia) {
    Kepler kep(MU_EARTH);
    Integrator<Kepler> integ(kep, IVPAlg::Vern8, 10.0);
    ...
    auto ref = read_julia_ref(julia_ref_path("vern8_twobody.bin"));

    for (int i = 0; i < 6; ++i) {
        EXPECT_NEAR(xf[i], ref.uf[i], 1e-8)
            << "Vern8 two-body state[" << i << "] mismatch: Tycho=" << xf[i]
            << " Julia=" << ref.uf[i];
    }
}

TEST_F(Vern8Test, CR3BPMatchesJulia) {
    CR3BP_Substitute ode(MU_CR3BP_SUBSTITUTE);
    Integrator<CR3BP_Substitute> integ(ode, IVPAlg::Vern8, 0.01);
    integ.set_abs_tol(1e-12);
    integ.set_rel_tol(1e-13);
    ...
    auto ref = read_julia_ref(julia_ref_path("vern8_crtbp.bin"));

    for (int i = 0; i < 6; ++i) {
        EXPECT_NEAR(xf[i], ref.uf[i], 1e-8)
            << "Vern8 CR3BP state[" << i << "] mismatch: Tycho=" << xf[i]
            << " Julia=" << ref.uf[i];
    }
}
