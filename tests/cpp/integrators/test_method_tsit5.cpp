...
TEST_F(Tsit5Test, TwoBodyMatchesJulia) {
    ...
    auto ref = read_julia_ref(julia_ref_path("tsit5_twobody.bin"));
...
TEST_F(Tsit5Test, CR3BPMatchesJulia) {
    ...
    Integrator<CR3BP_ODE> integ(ode, IVPAlg::Tsit5, 0.01);
    integ.set_abs_tol(1e-12);
    integ.set_rel_tol(1e-13);
    ...
    auto ref = read_julia_ref(julia_ref_path("tsit5_crtbp.bin"));
