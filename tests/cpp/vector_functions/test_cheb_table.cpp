...
TEST_F(ChebTableTest, ReproducesT3CoefficientsExactly) {
    ...
    oc::ChebTable::MatType vals(n + 1, 1); // (n+1) samples, 1 output channel
    for (int j = 0; j <= n; ++j)
        vals(j, 0) = Tk(3, pts[j]); // f = T_3 on [-1,1]
...
TEST_F(ChebTableTest, RoundTripReproducesNodeValues) {
    ...
    auto tab = oc::ChebTable::from_values(vals, lb, ub, n);
    for (int j = 0; j <= n; ++j)
        EXPECT_NEAR(tab.eval(pts[j])[0], vals(j, 0), 1e-12) << "node " << j;
}
...
TEST_F(ChebTableTest, MultiChannel) {
    ...
    const double lb = -1.0, ub = 1.0;
    auto pts = oc::ChebTable::cheb_points(n, lb, ub);
    oc::ChebTable::MatType vals(n + 1, 2);
    for (int j = 0; j <= n; ++j) {
        vals(j, 0) = Tk(2, pts[j]);
        vals(j, 1) = Tk(5, pts[j]);
    }
