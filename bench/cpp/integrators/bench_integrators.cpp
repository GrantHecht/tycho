...
static void BM_Integrate_SHO_DOPRI54(benchmark::State &state) {
    ...
    for (auto _ : state) {
        Integrator<SHO> integ(ode, tycho::IVPAlg::DOPRI54, 0.1);
        Eigen::Vector3d x0;
        x0 << 1.0, 0.0, 0.0;
...
static void BM_Integrate_SHO_DOPRI87(benchmark::State &state) {
    ...
    double tf = 2.0 * std::numbers::pi;
    for (auto _ : state) {
        Integrator<SHO> integ(ode, tycho::IVPAlg::DOPRI87, 0.1);
        Eigen::Vector3d x0;
        x0 << 1.0, 0.0, 0.0;
        auto xf = integ.integrate(x0, tf);
        benchmark::DoNotOptimize(xf);
...
static void BM_Integrate_SHO_FixedStep(benchmark::State &state) {
    ...
    double tf = 2.0 * std::numbers::pi;
    for (auto _ : state) {
        Integrator<SHO> integ(ode, tycho::IVPAlg::DOPRI87, 0.01);
...
static void BM_Integrate_Brach_DOPRI87(benchmark::State &state) {
    BrachODE ode(9.81);
    for (auto _ : state) {
        Integrator<BrachODE> integ(ode, tycho::IVPAlg::DOPRI87, 0.05);
        Eigen::VectorXd x0(5);
...
static void BM_Integrate_Brach_DOPRI54(benchmark::State &state) {
    BrachODE ode(9.81);
    for (auto _ : state) {
        Integrator<BrachODE> integ(ode, tycho::IVPAlg::DOPRI54, 0.05);
        Eigen::VectorXd x0(5);
