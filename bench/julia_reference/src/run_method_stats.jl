# Usage: julia --project=. src/run_method_stats.jl <method> <problem> <abstol> <reltol> <outfile>
# method  ∈ {Tsit5, BS3, BS5, Vern7, Vern8, Vern9, DP5, DP8}
# problem ∈ {two_body, crtbp}
# outfile: binary file receiving:
#   [int32 n | float64*n state | int32 naccept | int32 nreject | float64 walltime_sec]
#
# Uses out-of-place ODE formulation with StaticArrays (matches SP2 convention).
...
function main(args)
    length(args) == 5 || error("Usage: run_method_stats.jl <method> <problem> <abstol> <reltol> <outfile>")
    ...
    # Warm up JIT — do NOT measure this solve.
    ...
    open(outfile, "w") do io
        ...
        write(io, Int32(naccept))
        write(io, Int32(nreject))
        write(io, Float64(walltime))
    end
    @printf("%s %s (abs=%.1e rel=%.1e): uf=%s naccept=%d nreject=%d walltime=%.3es\n",
            args[1], args[2], abstol, reltol, collect(uf), naccept, nreject, walltime)
end
