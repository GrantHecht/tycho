...
and build the `_tychopy` extension `.so`; the committed nanobind stub
snapshot under `tychopy/_stubs/` is shipped via `install(FILES …)` —
stubs are *not* regenerated by the wheel build. The runtime `.so` must
still be import-compatible with the libstdc++ that originally produced
the snapshot, which is guaranteed when the `tycho` conda env is active
and the conda clang toolchain (above) is installed: scikit-build-core
picks up `CC`/`CXX` from the activated env and links against conda's
libstdc++. The wheel build does *not* go through `CMakePresets.json`
(scikit-build-core ignores presets), so toolchain selection is by env
var, not preset name. To regenerate stubs after a binding source
change, run `cmake --build build --target tychopy_stubs_snapshot` from
a conda-toolchain build directory and commit the diff.
